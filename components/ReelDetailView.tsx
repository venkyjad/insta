...
export default function ReelDetailView({ reel, username, onClose }: ReelDetailViewProps) {
  ...
  const handleRepurpose = async (formData: RepurposingFormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || data.error || 'Failed to repurpose content');
      }
          ...
          {activeTab === 'repurpose' && (
            <div>
              ...
              {repurposingError && (
                <div className="p-6">
                  <div className="p-4 bg-red-950 border border-red-900 rounded-lg">
                    <p className="text-red-300 text-sm">{repurposingError}</p>
                  </div>
                </div>
              )}
                  ...
                  {repurposedContent.visualSuggestions && repurposedContent.visualSuggestions.length > 0 && (
                    <div>
                      ...
                      <div className="bg-neutral-950 border border-neutral-800 rounded-lg p-4 space-y-2">
                        {repurposedContent.visualSuggestions.map((suggestion: string, idx: number) => (
                          <div key={idx} className="flex items-start gap-2">
                            <span className="text-neutral-500 text-xs mt-0.5">{idx + 1}.</span>
                            <p className="text-neutral-300 text-sm flex-1">{suggestion}</p>
                          </div>
                        ))}
                      </div>
                    </div>
